<?php
namespace Reverb\ReverbSync\Model\Adapter;
class Curl
{
    ...
    const USER_AGENT_TEMPLATE = 'Reverb-Magento ReverbMagentoVersion=1.0.0 MagentoVersion=%s MagentoDomain=%s';
    ...
    public function __construct(
        ...
        \Reverb\Base\Helper\Data $baseHelper,
    ...
    protected function _addCurrentMagentoVersionUserAgent()
    {
        ...
        $magento_base_web_url = $this->_storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_WEB); 

        $magento_version_string = sprintf(self::MAGENTO_VERSION_TEMPLATE, $magento_version);

        $magento_domain = $this->_baseHelper->extractDomainFromUrl($magento_base_web_url);
